...
public class CouchMover
{
  ...
  private static final int MAX_COPY_BUFFER_SIZE = 128 * 1024; 
  ...
  private HTTPURLResponse serverHTTPURLResponse(String urlString, String httpMethod, InputStream dataStream, String contentType, boolean includeHeaders)
  {
    ...
    catch (IOException e) { e.printStackTrace(); }  // sending request

    try  // parsing response json
    {
      JSONObject json = ((responseString==null) || (responseString.length() == 0)) ? new JSONObject() : new JSONObject(responseString);
      return new HTTPURLResponse(responseString, json, statusCode, headers);
    } 
    catch (JSONException e) { e.printStackTrace(); } // parsing response json
  ...
  {
    try { return (data.json.getString(field).compareTo(value)==0); } 
    catch (JSONException e) { /*it is OK TO not exist*/ }
    return false;
  }

  boolean responseDataHasField(HTTPURLResponse data, String field)
  {
    try { return (data.json.get(field)!=null); } 
    catch (JSONException e) { /*it is OK TO not exist*/ }
    return false;
  }
  ...
  {
    try { return data.json.getString(field); } 
    catch (JSONException e) { /*it is OK TO not exist*/ }
    return null;
  }
}
